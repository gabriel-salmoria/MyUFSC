...
async function scrapeAndGetMasterSchedule(): Promise<{
  data: any;
  semester: string;
}> {
  console.log("--- Step 1: Scraping Master Schedule ---");

  const internalDataDir = path.join(PROJECT_ROOT, "data", "schedule");
  ...
  try {
    ...
    console.error(
      "Failed to run cargo scraper. Make sure Rust/Cargo is installed and you are online.",
    );
    throw e;
  }
  ...
  console.log(
    `Selected latest semester: ${latest.fullCode} (${latest.filename})`,
  );
...
function filterScheduleForCurriculum(
  masterSchedule: any,
...
async function main() {
  ...
  try {
    ...
    const { data: masterSchedule, semester } =
      await scrapeAndGetMasterSchedule();
    ...
    if (targetProgramId) {
      ...
      res = await client.query(
        'SELECT * FROM curriculums WHERE "programId" = $1',
        [targetProgramId],
      );
    } else {
    ...
    for (const row of res.rows) {
      ...
      const filteredSchedule = filterScheduleForCurriculum(
        masterSchedule,
        courseIds,
      );
